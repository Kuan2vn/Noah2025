...
for dataset_info in dataset_config:
    ...
    try:
        ...
        if dataset_name == 'oxford_iiit_pet':
            ...
            # For KITTI, need to use the specific task
            task = dataset_params.get('config', 'closest_vehicle_distance')
            
            # Try loading the dataset with task config
            try:
                train_val = tfds.load(dataset_name, split='train', as_supervised=False)
                test = tfds.load(dataset_name, split='test', as_supervised=False)
                ...
                # For KITTI, determine the label based on the task
                def preprocess_fn(item):
                    image = item['image']
                    if task == 'closest_vehicle_distance':
                        # Transform the distance target to a classification task
                        # (This is a simplified version, might need adjustment)
                        distances = [obj['truncated'] for obj in item['objects']]
                        if not distances:
                            label = 0  # No vehicle
                        else:
                            label = int(min(distances) * 10)  # Discretize distance
                    else:
                        # Default to first object class
                        label = item['objects'][0]['type'] if item['objects'] else 0
                ...
                print("You might need to download KITTI manually.")
